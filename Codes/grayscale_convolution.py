...
def convolution(img,kernel,center):
    ...
    bordered_image = cv2.copyMakeBorder(src=img, top=pad_top, bottom=pad_bottom, left=pad_left, right=pad_right, borderType=cv2.BORDER_CONSTANT)
    print(bordered_image)

    output = np.zeros_like(bordered_image, dtype='float32')

    # height and width of bordered image
